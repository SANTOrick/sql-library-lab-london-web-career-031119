...
def select_name_and_series_subgenres_of_authors
  "SELECT subgenres.name, subgenres.author"
end

def select_series_title_with_most_human_characters
  "SELECT series.title FROM series JOIN characters ON characters.series_id = series.id WHERE species = 'human' GROUP BY species ORDER BY species DESC LIMIT 1"
end
